...
void UserInterface::collUpdateFileList(wxCommandEvent &event) {
	...
	for (int j = 0; j < rule->getContentCount(RULE_FILES); j++) {
		...
		collFileList->SetItem(i, 1, Util::hex2str(content->getMD5(), MD5_DIGEST_LENGTH));
...
void UserInterface::collUpdateParamList(wxCommandEvent &event) {
	...
	for (int j = 0; j < rule->getContentCount(RULE_PARAMETERS); j++) {
		...
		switch(content->getParamType()) {
			case PARAM_LONG:
				collParamList->SetItem(index, 1, wxString::Format(wxT("%ld"), content->getParam().latom));
				break;
			case PARAM_DOUBLE:
				collParamList->SetItem(index, 1, wxString::Format(wxT("%lf"), content->getParam().datom));
				break;
			case PARAM_STRING:
